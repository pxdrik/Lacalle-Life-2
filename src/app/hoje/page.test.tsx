 ...
 *
 * 28/08/2026 — movido de `app/page.test.tsx` junto com a tela: `/` virou a
 * Landing Page pública, e este é o teste da tela que era o antigo `/`, agora
 * em `/hoje`. Nada no teste mudou, só o endereço.
 */
...
describe("hydrating the /hoje page across a day boundary", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it("renders the same markup on the server regardless of the server's clock", () => {
    ...
    const late = renderToStaticMarkup(<HojePage />);

    vi.setSystemTime(new Date(2026, 7, 28, 1, 0));
    const early = renderToStaticMarkup(<HojePage />);
  ...
  it("hydrates without warning even when the browser's clock is already a day ahead of the server's, then fills in the real day", async () => {
    ...
    vi.setSystemTime(new Date(2026, 7, 27, 23, 0));
    const serverHtml = renderToStaticMarkup(<HojePage />);
    ...
    act(() => {
      hydrateRoot(container, <HojePage />);
    });
